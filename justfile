...
APP_BUILD := env_var_or_default('APP_BUILD', `git rev-parse --short HEAD`)  
...
@dev-services: 
	{{ if WITH_DOCKER != "no" { "docker-compose up -d db search || echo \"WARNING: You may want to make sure the docker daemon is started or run 'colima start' first.\"" } else {"Skipping docker services"} }}
